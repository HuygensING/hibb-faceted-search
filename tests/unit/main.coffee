...
describe 'View : Main ::', ->
	...
	describe 'initialize :::', ->
		it 'should extend config with options', ->
			...
			mainView.config.has('myattr').should.be.ok
			mainView.config.get('autoSearch').should.not.be.ok
			mainView.config.get('queryOptions').fuzzy.should.be.ok
			mainView.config.get('queryOptions').should.have.ownProperty 'nonExistentAttr'
	...
	describe 'render :::', ->
		...
		it 'should call initFacets with facetViewMap', ->
			mainView.initFacets = setup.sinon.spy()
			mainView.initialize
				facetViewMap:
					FACETTYPE: 'someViewClass'
			mainView.initFacets.should.have.been.calledWith FACETTYPE: 'someViewClass'
	...
	describe 'renderTextView :::', ->
		it 'should attach textSearch to mainView', ->
			mainView.$('.text-search').find('.search-input').length.should.equal 1
	...
	describe 'onSwitchType :::', ->
		...
		it 'should set config.textSearch to simple if config.textSearch is advanced', ->
			stub = setup.sinon.stub mainView, 'search'
			mainView.config.set textSearch: 'advanced'
			mainView.onSwitchType ev
			mainView.config.get('textSearch').should.equal 'simple'

		it 'should set config.textSearch to advanced if config.textSearch is simple', ->
			stub = setup.sinon.stub mainView, 'search'
			mainView.config.set textSearch: 'simple'
			mainView.onSwitchType ev
			mainView.config.get('textSearch').should.equal 'advanced'
...
#			mainView.facets.update.should.have.been.called
	...
	describe 'sortResultsBy :::', ->
		it 'should call set on queryOptions with the sort field', ->
			arg = [
				fieldname: 'toetersenbellen'
				direction: 'asc'
			,
				fieldname: 'toetersenbellen2'
				direction: 'desc'
			]
			stub = setup.sinon.stub mainView.queryOptions, 'set'
			mainView.sortResultsBy arg
			stub.should.have.been.calledWith
				resultFields: ['toetersenbellen', 'toetersenbellen2']
				sortParameters: arg
